...
def leaderboard_entries(conn, limit=None):
    ...
    for row in rows:
        ...
        total_hours = hours * 0.20
        row['total_hours'] = round(hours, 2)
        row['reward_score'] = round(total_reward, 2)
        row['hours_score'] = round(total_hours, 2)
...
def index():
    ...
    user = fetch_user(conn, session.get('user_id')) if session.get('user_id') else None
    conn.close()
...
def leaderboard():
    conn = get_db()
    rankings = leaderboard_entries(conn)
    user = fetch_user(conn, session['user_id']) if session.get('user_id') else None
    notifications = []
